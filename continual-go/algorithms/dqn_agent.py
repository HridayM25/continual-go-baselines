...
class AgentState:
    ...
    buffer_state: Any     
    init_params : Any                             # (location, full, buffers)
...
def init_agent_state(agent_config: DQNConfig,action_dim: int,obs_shape: tuple,rng: jax.random.PRNGKey):
    ...
    #     print(jax.tree_util.keystr(path), leaf.shape)
    # sys.exit()

    tx = optax.adam(learning_rate=agent_config.q_lr)

    # [TODO] : Add trac config for enabling/disabling TRAC
    # tx = start_trac(optax.adam(1e-3))
        ...
        def _loss_fn(params):
            ...
            def _wasserstein2(path,p,q):
                if path[-1].key == 'kernel' : 
                    return jnp.sum((jnp.sort(p.ravel()) - jnp.sort(q.ravel()))**2)
            ...
            l2_init_penalty = cfg.l2_init_weight * sum(tree_leaves(tree_map(lambda p,q : jnp.sum((p - q) ** 2), params, init_params)))
            wasserstein2_penalty = cfg.w2_weight * sum(tree_leaves(tree_map_with_path(_wasserstein2, params, init_params)))

            return jnp.mean((chosen_q - target) ** 2) + l2_init_penalty + wasserstein2_penalty
